...
export default function ChartPage({ onGoToProfile }: { onGoToProfile: () => void }) {
  ...
  useEffect(() => {
    ...
    // 如果已经有了分析结果，且 profile 信息没变，就不再重新检查或获取
    // 这里的 id 和 birth_date 是关键
    if (deepAnalysis && !isAnalyzing) return;

    // 规格化出生日期字符串，确保缓存 Key 稳定
    const normalizedBirth = profile.birth_date ? new Date(profile.birth_date).toISOString() : '';
    const cacheKey = `bazi_v2_analysis_${profile.id}_${normalizedBirth}`;
    ...
    async function fetchDeepAnalysis() {
      setIsAnalyzing(true);
      try {
        const trigger = `__GENERATE_BAZI_ANALYSIS__:${baziData?.pillars[2].stem}`;
        setDeepAnalysis("");
        const stream = askAIStream('/api/ai/fortune', { query: trigger, context: null, title: "命盘简评", save_history: false });
        let fullContent = "";
        let firstChunk = true;
