...
import { Sparkles, Briefcase, Wallet, Heart, BrainCircuit, X, MessageCircle, Send, Trash2, User } from 'lucide-react';
...
export default function FortunePage() {
  ...
  useEffect(() => {
    ...
    if (todayFortuneHistory.length > 0) {
      // 只有在当前没有正在发送消息且对话为空（初始状态）时，才从历史记录同步
      if (chatMessages.length > 0 && !isAiLoading) return;
  ...
  useEffect(() => {
    ...
    if (spiritDiary && !isDiaryLoading) return;
    ...
    async function fetchDiary() {
      ...
      try {
        const trigger = `__GENERATE_SPIRIT_DIARY__:${lunar.getDayInGanZhi()}:${baziData?.pillars[2].stem}`;
        setSpiritDiary("");
        const stream = askAIStream('/api/ai/fortune', { query: trigger, context: null, title: "今日灵启", save_history: false });
        let fullContent = "";
        let firstChunk = true;
  ...
  return (
    <div className="space-y-10 pb-10">
      ...
      {/* AI Spirit Diary */}
      <section className="bg-surface-container-low p-8 rounded-[2rem] border border-primary/5 shadow-xl relative">
        ...
        {(isDiaryLoading || (spiritDiary === null)) ? (
          ...
          <div className="relative prose prose-invert prose-sm max-w-none 
            ...
            prose-li:my-1 prose-ul:my-2 prose-ul:pl-4
            prose-headings:text-primary prose-headings:my-3">
            {spiritDiary ? (
              <MarkdownContent content={spiritDiary} />
            ) : (
              <p className="text-on-surface-variant/60 italic">完善生辰信息，开启每日指引。</p>
            )}
          </div>
        )}
      ...
      {chatMessages.length > 0 && (
        <section className="flex flex-col gap-8 pt-8 pb-64">
          ...
          <div className="flex flex-col gap-8">
            {chatMessages.map((msg, idx) => (
              ...
              >
                <div className={cn(
                  "w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm border border-outline-variant/5",
                ...
                <div className={cn(
                  "p-5 rounded-[1.5rem] max-w-[85%] text-sm leading-relaxed shadow-lg relative",
                  msg.role === 'user' 
                  ...
                  {msg.role === 'user' ? (
                    ...
                    <div className="prose prose-invert prose-sm max-w-none 
                      ...
                      prose-li:my-1 prose-ul:my-2 prose-ul:pl-4
                      prose-headings:text-primary prose-headings:text-base prose-headings:my-3">
